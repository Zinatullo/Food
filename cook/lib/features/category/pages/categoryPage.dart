...
class CategoryPage extends StatefulWidget {
  ...
  const CategoryPage({super.key, required this.categoryName, this.item});
...
class _CategoryPageState extends State<CategoryPage> {
  ...
  void initState() {
    super.initState();
    _itemsFuture = _foodService.getProductsByCategory(widget.categoryName);
  }
  ...
  Widget build(BuildContext context) {
    final List<String> categories = FoodService.getCategories();
              ...
              children: [
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (String newCategory) {
                    if (newCategory == widget.categoryName) return;

                    context.pushReplacement(
                      '/category?name=${Uri.encodeComponent(newCategory)}',
                    );
                  },
                    ...
                    return categories.map((String cat) {
                      return PopupMenuItem<String>(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: cat == widget.categoryName
                                ? FontWeight.w700
                                : FontWeight.normal,
                            color: cat == widget.categoryName
  ...
  child: Column(
    ...
    children: [
      ...
      const SizedBox(height: 20),
      // Рестораны — уникальные по restaurantName, берём первый товар каждого
      Builder(
        builder: (context) {
          final Map<String, FoodItem> uniqueByRestaurant = {
            for (final food in items) food.restaurantName: food,
          };
          final restaurants = uniqueByRestaurant.values.toList();
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              return Padding(
                ...
                child: RestaurantHomeCard(item: restaurants[index]),
              );
            },
          );
        },
